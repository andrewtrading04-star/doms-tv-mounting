...
(function () {
  ...
  function wire(root){
    ...
    root.querySelectorAll('.dom-inc').forEach(b=>b.addEventListener('click',()=>{setQty(b.dataset.s,b.dataset.o,getQty(b.dataset.s,b.dataset.o)+1);render();}));
    root.querySelectorAll('.dom-dec').forEach(b=>b.addEventListener('click',()=>{setQty(b.dataset.s,b.dataset.o,Math.max(0,getQty(b.dataset.s,b.dataset.o)-1));render();}));
    root.querySelectorAll('.dom-sel').forEach(c=>c.addEventListener('click',()=>{selectOnly(c.dataset.s,c.dataset.o);render();}));
    ...
    root.querySelectorAll('.dom-comment').forEach(t=>t.addEventListener('input',e=>{optionComments[e.target.dataset.o]=e.target.value;}));

    // ZIP verification — update button/message in-place so input keeps focus
    const zipInput=root.querySelector('#verify-zip');
    if(zipInput){
      zipInput.addEventListener('input',e=>{
        const z=e.target.value.replace(/\D/g,'');
        if(e.target.value!==z) e.target.value=z; // strip non-digits live
        customer.zip=z;
        const isValid=/^\d{5}$/.test(z);
        const inArea=isValid&&DENVER_ZIPS.has(z);
        const notInArea=isValid&&!inArea;
        const msg=root.querySelector('#zip-msg');
        const btn=root.querySelector('#btn-verify-zip');
        if(msg){
          if(inArea) msg.innerHTML=`<div style="background:#D6E8F7!important;border:1px solid ${BLUE_LIGHT}!important;border-radius:8px!important;padding:12px!important;font-size:13px!important;color:${BLUE_DK}!important;"><strong>&#10003; Great news!</strong> We service your area.</div>`;
          else if(notInArea) msg.innerHTML=`<div style="background:#FFE5E5!important;border:1px solid #FF9999!important;border-radius:8px!important;padding:12px!important;font-size:13px!important;color:#C53030!important;"><strong>Outside our service area.</strong> We currently serve Denver &amp; the surrounding metro.</div>`;
          else msg.innerHTML='';
        }
        if(btn){btn.disabled=!inArea;btn.style.cssText=inArea?S.btnPri:S.btnDis;}
      });
    }
    const verifyBtn=root.querySelector('#btn-verify-zip');
    if(verifyBtn){
      verifyBtn.addEventListener('click',()=>{if(DENVER_ZIPS.has(customer.zip)){zipVerified=true;stepIdx=1;render();}});
    }
    const backZipBtn=root.querySelector('#btn-back-zip');
    if(backZipBtn){
      backZipBtn.addEventListener('click',()=>{zipVerified=false;selectedService=null;stepIdx=0;selections={};selectedSlot=null;describeText='';render();});
    }
  }
  ...
  function goNext(){
    const fs=flowSteps();
    let ni=stepIdx+1;
    while(ni<fs.length&&shouldSkip(fs[ni]))ni++;
    if(fs[ni]==='slots')fetchSlots();
    if(selectedService==='tv'&&isFrameTV)setQty(serviceConfig.frameBracketSectionId,serviceConfig.frameBracketOptionId,(selections['__frame_type']||[]).includes('regular')?0:totalTVs());
    stepIdx=ni; render();
  }
  function goBack(){
    const fs=flowSteps();
    let pi=stepIdx-1;
    while(pi>0&&shouldSkip(fs[pi]))pi--;
  ...
  async function fetchSlots(){
    try{
      const r=await fetch(`${API_BASE}/slots?territory_id=${TERRITORY_ID}&duration=120&days=30`);
      const d=await r.json();
      slotsByDate={};calYear=null;calMonth=null;
      for(const day of(d.days||[]))slotsByDate[day.date]=(day.timeslots||[]).map(sl=>({id:sl.id,arrival_window:sl.formatted}));
  ...
  async function doSubmit(root){
    ...
    if(!customer.first_name)return alert('Please enter your first name.');
    ...
    const restore=()=>{if(submitBtn){submitBtn.textContent=isTV?'Complete My Booking ✓':(QUOTES[selectedService].cta+' ✓');submitBtn.disabled=false;}};

    // Tokenize card for TV bookings (validation/hold). Skipped if Stripe key not set.
    if(isTV&&_stripe&&_stripeCard){
      if(submitBtn){submitBtn.textContent='Processing…';submitBtn.disabled=true;}
      const{error}=await _stripe.createPaymentMethod({type:'card',card:_stripeCard,billing_details:{name:`${customer.first_name} ${customer.last_name}`.trim(),email:customer.email,phone:customer.phone,address:{line1:customer.address,postal_code:customer.zip}}});
      if(error){restore();return alert(error.message);}
    }

    // Build Zenbooker selections
    let zbk_selections=[];
    if(isTV){
      const multi=new Set(['qty_multi','qty_match','multi_select']);
      zbk_selections=serviceConfig.sections.map(sec=>{
        ...
        return multi.has(sec.type)?{section_id:sec.id,selected_options:opts}:{section_id:sec.id,option_id:opts[0].option_id};
      }).filter(Boolean);
    }else{
      const q=QUOTES[selectedService];
      zbk_selections=[{section_id:q.sectionId,answer:describeText.trim()}];
    }

    const payload={
      kind:isTV?'booking':'quote',
      territory_id:TERRITORY_ID,
      service_id:isTV?serviceConfig.service_id:QUOTES[selectedService].service_id,
      selectedSlot:isTV?selectedSlot:null,
      customer:{...customer},
      city:LOCATION.city, state:LOCATION.state, postal_code:customer.zip,
      zbk_selections, tip:isTV?tipAmount:0,
    };

    if(submitBtn){submitBtn.textContent=isTV?'Booking…':'Sending…';submitBtn.disabled=true;}
    try{
      const r=await fetch(`${API_BASE}/book`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      if(r.ok){
        try{
          const res=await r.json().catch(()=>({}));
          const slot=isTV?((slotsByDate[selectedDate]||[]).find(s=>s.id===selectedSlot)||{}):{};
          const df=isTV&&selectedDate?fmtDate(selectedDate):null;
          const lines=[];
          if(isTV)for(const sec of serviceConfig.sections)for(const sel of(selections[sec.id]||[])){
            const opt=sec.options.find(o=>o.id===sel.option_id);
            if(opt&&sel.quantity>0)lines.push({label:opt.label,qty:sel.quantity,amount:(opt.price||0)*sel.quantity});
          }
          localStorage.setItem('doms_booking',JSON.stringify({
            kind:isTV?'booking':'quote',
            service:selectedService,
            firstName:customer.first_name||'',
            name:`${customer.first_name||''} ${customer.last_name||''}`.trim(),
            email:customer.email||'', phone:customer.phone||'',
            address:customer.address||'', city:LOCATION.city, state:LOCATION.state, zip:customer.zip||'',
            dateISO:isTV?(selectedDate||''):'', dateLong:df?`${df.long}, ${df.date}`:'',
            timeWindow:slot.arrival_window||'',
            describe:isTV?'':describeText.trim(),
            lines, total:isTV?calcTotal():0, tip:isTV?(tipAmount||0):0,
    ...
    }catch{restore();alert('Connection error. Please try again.');}
  }
  ...
  function boot(){
    if(!document.getElementById('doms-widget-style')){
      const s=document.createElement('style');
      s.id='doms-widget-style';
      s.textContent='@media(min-width:768px){#doms-widget{max-width:638px!important;}}';
      document.head.appendChild(s);
    }
