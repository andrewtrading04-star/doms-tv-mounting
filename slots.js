// /api/slots.js — returns available Zenbooker timeslots for Dom's territory.
export default async function handler(req, res) {
  ...
  if (!ZBK_KEY) {
    return res.status(500).json({ error: 'ZENBOOKER_API_KEY missing' });
  }
  ...
  try {
    ...
    if (!r.ok) {
      console.error('[slots] Zenbooker error:', r.status, data);
